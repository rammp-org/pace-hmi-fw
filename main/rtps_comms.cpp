...
std::vector<uint8_t> serialize_adc(float x, float y, float twist, uint32_t buttons,
                                   uint32_t drive_mode) {
  auto bytes = cdr::serialize<cdr::xcdr1>(rammp_adc_xy_twist_t{x, y, twist, buttons, drive_mode});
  return bytes ? to_uint8(*bytes) : std::vector<uint8_t>{};
}
...
bool rtps_comms_publish_adc(float x, float y, float twist, uint32_t buttons, uint32_t drive_mode) {
  ...
  return participant->publish(kAdcTopic, serialize_adc(x, y, twist, buttons, drive_mode));
}
