...
namespace espp {
/**
 *  @brief 2-axis Joystick with axis mapping / calibration, plus an optional
 *         independent third (Z) axis.
 *
 ...
 *
 * @note The optional Z axis (twist / throttle / rudder) is mapped by its own
 *       RangeMapper and is deliberately *not* part of the X/Y gimbal: the
 *       Type::CIRCULAR deadzone and magnitude clamping apply to the X/Y vector
 *       only. Physically a twist pot is a separate potentiometer, not a third
 *       dimension of the stick's tilt, so a spherical deadzone across all three
 *       would be wrong. Z therefore takes its center / range deadbands from its
 *       own FloatRangeMapper::Config.
 */
...
public:
  ...
  /**
   * @brief function for getting x/y/z values for a joystick which has a third
   *        (twist / throttle) axis.
   * @param x Pointer to the x value. Function should fill this variable
   *          with the latest reading.
   * @param y Pointer to the y value. Function should fill this variable
   *          with the latest reading.
   * @param z Pointer to the z value. Function should fill this variable
   *          with the latest reading.
   * @return True if the values were able to be retrieved, false otherwise.
   */
  typedef std::function<bool(float *x, float *y, float *z)> get_values_3d_fn;
  ...
  struct Config {
    ...
    std::optional<espp::FloatRangeMapper::Config> z_calibration{
        std::nullopt}; /**< Optional configuration for an independent third axis
                          (twist / throttle). When unset the joystick is a plain
                          2-axis joystick and z() always returns 0. */
    ...
    espp::Joystick::get_values_fn get_values{nullptr};       /**< Function to retrieve the latest
                                                ...
                                                raw_y). */
    espp::Joystick::get_values_3d_fn get_values_3d{nullptr}; /**< Function to retrieve the latest
                                          unmapped x/y/z values. Takes precedence over get_values
                                          when set. Unused if you call update(float raw_x, float
                                          raw_y, float raw_z). */
   ...
   *        independently.
   *  @note The Z axis calibration is never modified by this function - Z is
   *        independent of the X/Y gimbal type.
  ...
  /**
   * @brief Update the z axis mapping.
   * @param z_calibration New z-axis range mapping configuration to use. Enables
   *        the z axis if it was not previously configured.
   * @note Unlike the x/y calibrations, the z calibration's deadbands are used
   *       as-is regardless of the joystick Type, since z is independent of the
   *       X/Y gimbal.
   */
  void set_z_calibration(const espp::FloatRangeMapper::Config &z_calibration);

  /**
   * @brief Whether this joystick was configured with a third (z) axis.
   * @return True if a z calibration has been provided.
   */
  bool has_z() const;
   ...
   *        position.
   * @note Requires that the get_values_ or get_values_3d_ function is set.
   *       get_values_3d_ takes precedence when both are set.
   */
  void update();
   ...
   * @note Leaves any configured z axis at its previous value.
   */
  void update(float raw_x, float raw_y);

  /**
   * @brief Update the joystick's position using the provided raw x, y and z
   *        values.
   * @param raw_x The raw x-axis value.
   * @param raw_y The raw y-axis value.
   * @param raw_z The raw z-axis value. Ignored if no z calibration was
   *        configured.
   */
  void update(float raw_x, float raw_y, float raw_z);
  ...
  /**
   * @brief Get the most recently updated z axis calibrated position.
   * @return The most recent z-axis position (from when update() was last
   *         called), or 0 if no z axis was configured.
   */
  float z() const;
   ...
   * @note Covers the X/Y gimbal only; use z() for the third axis.
   */
  ...
  /**
   * @brief Get the most recently updated raw / uncalibrated z reading.
   * @return The most recent raw z measurement (from when update() was last
   *         called), or 0 if no z axis was configured.
   */
  float raw_z() const;
...
/**
 * @brief Self-check for the Joystick mapping math. Aborts (assert) on failure.
 * @note Cheap enough to call once at boot; it constructs a couple of joysticks
 *       and exercises center / range / deadzone / circular-clamp behaviour plus
 *       the independence of the z axis from the X/Y gimbal.
 * @return True if all checks passed.
 */
bool joystick_selftest();
