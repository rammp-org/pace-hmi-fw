...
static void button_panel_observer(lv_observer_t *observer, lv_subject_t *subject) {
  ...
  lv_obj_set_style_bg_color(panel,
                            lv_subject_get_int(subject) ? lv_palette_main(LV_PALETTE_BLUE)
                                                        : lv_color_hex((uint32_t)ui_get_theme_value(
...
static void speed_label_observer(lv_observer_t *observer, lv_subject_t *subject) {
  ...
  if (tenths < 0)
    tenths = 0;
  if (tenths > RAMMP_SPEED_MAX_TENTHS)
    tenths = RAMMP_SPEED_MAX_TENTHS;
  lv_label_set_text_fmt(lv_observer_get_target_obj(observer), "%d.%d", (int)(tenths / 10),
                        (int)(tenths % 10));
}
...
static void style_focus(lv_obj_t *button) {
  ...
  ui_object_set_themeable_style_property(button, focused, LV_STYLE_BORDER_COLOR,
                                         _ui_theme_color_focused);
  ui_object_set_themeable_style_property(button, focused, LV_STYLE_BORDER_OPA,
...
void sim_nav_on_stick_sample(void) {
  ...
  /* ---- Held-direction latch for the hold gestures (see nav_key above) -- */
  {
    ...
    if (!nav_engaged)
      ...
      nav_key = x > 0 ? LV_KEY_RIGHT : LV_KEY_LEFT;
    else
      nav_key = y > 0 ? LV_KEY_UP : LV_KEY_DOWN;
  }
