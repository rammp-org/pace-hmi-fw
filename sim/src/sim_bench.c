...
static const struct {
  ...
  bool momentary; /* acts on the press edge rather than being held */
    ...
    {"Not assigned", false, false}, {"Joystick button", true, false},
    {"Home / back", false, true},   {"Next drive mode", false, true},
    {"Horn", false, true},          {"Lights", false, true},
};
...
static void invoke(const bench_button_t *b) {
  ...
  case FN_HOME:
    sim_nav_go_home();
    break;
  case FN_DRIVE_PROFILE:
    sim_nav_next_drive_profile();
    break;
  case FN_HORN:
  case FN_LIGHTS:
...
static void caption_refresh(bench_button_t *b) {
  ...
  lv_label_set_text_fmt(b->caption, "%s  %s", b->wiring, real ? "in firmware" : "proposal");
  lv_obj_set_style_text_color(b->caption, lv_color_hex(real ? COLOR_REAL : COLOR_MOCKUP),
                              LV_PART_MAIN);
}
...
bool sim_bench_joystick_button(void) {
  ...
  for (i = 0; i < BUTTON_COUNT; i++) {
    if (buttons[i].fn == FN_JOYSTICK && buttons[i].held && mouse_down)
      return true;
  }
  return false;
}
