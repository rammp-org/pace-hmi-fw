...
brings up exactly one `espp::Button` and `messages/joystick_message.hpp` defines exactly one
button bit.
...
ways round. When a mapping earns its place, put it in `messages/joystick_message.hpp` and
